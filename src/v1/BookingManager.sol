 ...
 *
 * Features:
 * - Create COMMITMENT_FEE bookings
...
contract BookingManager is ReentrancyGuard, AccessControl, EIP712 {
    ...
    /**
     * @dev Creates a new COMMITMENT_FEE booking (direct call)
    ...
    /**
     * @dev Creates a new COMMITMENT_FEE booking via meta-transaction (relayer)
    ...
    function createBookingByRelayer(
        ...
        // Verify nonce and deadline first (cheaper checks before expensive operations)
        if (block.timestamp > req.deadline) revert SignatureExpired();
        if (req.to != address(this)) revert InvalidRequest();
        ...
        // Increment nonce AFTER all validations pass
        nonces[req.from]++;

        // Decode function call from data
        (address mentor, uint256 sessionTime, uint256 amount) = abi.decode(
            req.data[4:],
    ...
    /**
     * @dev Claims mentor payment via relayer
     * @param req Forward request
     * @param signature Signature from mentor
     */
    function claimMentorPaymentByRelayer(
        ForwardRequest calldata req,
        bytes calldata signature
    ) external nonReentrant {
        // Verify nonce and deadline first
        if (block.timestamp > req.deadline) revert SignatureExpired();
        if (req.to != address(this)) revert InvalidRequest();
        if (nonces[req.from] != req.nonce) revert InvalidNonce();
        ...
        // Decode bookingId from data
        uint256 bookingId = abi.decode(req.data[4:], (uint256));

        // Execute claim
        _claimMentorPayment(req.from, bookingId);
    }

    /**
     * @dev Claims mentee refund via relayer
     * @param req Forward request
     * @param signature Signature from mentee
     */
    function claimMenteeRefundByRelayer(
        ForwardRequest calldata req,
        bytes calldata signature
    ) external nonReentrant {
        // Verify nonce and deadline first
        ...
        _claimMenteeRefund(req.from, bookingId);
    }

    /**
     * @dev Cancels booking via relayer
     * @param req Forward request
     * @param signature Signature from mentee
     */
    function cancelBookingByRelayer(
        ForwardRequest calldata req,
        bytes calldata signature
    ) external nonReentrant {
        // Verify nonce and deadline first
        ...
        _cancelBooking(req.from, bookingId);
    }

    /**
     * @dev Internal function to create COMMITMENT_FEE booking
    ...
    /**
     * @dev Confirms session attendance
    ...
    function claimMentorPayment(uint256 bookingId) external nonReentrant {
        _claimMentorPayment(msg.sender, bookingId);
    }

    /**
     * @dev Internal function to process mentor payment claim
     * @param claimer Claimer address
     * @param bookingId Booking identifier
     */
    function _claimMentorPayment(address claimer, uint256 bookingId) internal {
        Booking storage booking = bookings[bookingId];
        if (claimer != booking.mentor) revert NotMentor();
        if (
    ...
    function claimMenteeRefund(uint256 bookingId) external nonReentrant {
        _claimMenteeRefund(msg.sender, bookingId);
    }

    /**
     * @dev Internal function to process mentee refund claim
     * @param claimer Claimer address
     * @param bookingId Booking identifier
     */
    function _claimMenteeRefund(address claimer, uint256 bookingId) internal {
        Booking storage booking = bookings[bookingId];
        if (claimer != booking.mentee) revert NotMentee();
    ...
    function cancelBooking(uint256 bookingId) external nonReentrant {
        _cancelBooking(msg.sender, bookingId);
    }

    /**
     * @dev Internal function to cancel a booking
     * @param canceller Canceller address
     * @param bookingId Booking identifier
     */
    function _cancelBooking(address canceller, uint256 bookingId) internal {
        Booking storage booking = bookings[bookingId];
        if (canceller != booking.mentee) revert OnlyMenteeCanCancel();
